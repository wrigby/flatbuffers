...
inline bool IsTypedVectorElementType(Type t) {
  return t >= TYPE_INT && t <= TYPE_KEY;
}

inline bool IsTypedVector(Type t) {
  return t >= TYPE_VECTOR_INT && t <= TYPE_VECTOR_KEY;
}
...
class Reference {
 public:
  ...
  bool IsNull() { return type_ == TYPE_NULL; }
  bool IsInt() { return type_ == TYPE_INT || type_ == TYPE_INDIRECT_INT; }
  bool IsUInt() { return type_ == TYPE_UINT || type_ == TYPE_INDIRECT_UINT;; }
  bool IsIntOrUint() { return IsInt() || IsUInt(); }
  bool IsFloat() { return type_ == TYPE_FLOAT || type_ == TYPE_INDIRECT_FLOAT; }
  bool IsNumeric() { return IsIntOrUint() || IsFloat(); }
  bool IsString() { return type_ == TYPE_STRING; }
  bool IsKey() { return type_ == TYPE_KEY; }
  bool IsVector() { return type_ == TYPE_VECTOR || type_ == TYPE_MAP; }
  bool IsMap() { return type_ == TYPE_MAP; }
...
inline Reference Vector::operator[](size_t i) const  {
  auto len = size();
  // TODO(wvo): This should really be an assert instead?
